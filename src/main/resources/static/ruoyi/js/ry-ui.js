        ...
        operate: {
			...
			chengji: function(id) {
				...
				if($.common.isEmpty(id) && table.options.type == table_type.bootstrapTreeTable) {
					...
					var url = table.options.chengjiUrl.replace("{id}", row[table.options.uniqueId]);
					$.modal.open("评分" + table.options.modalName, url);
				} else {
					$.modal.open("评分" + table.options.modalName, $.operate.chengjiUrl(id));
				}
			},
			// 评分信息，以tab页展现
			chengjiTab: function(id) {
				table.set();
				$.modal.openTab("评分" + table.options.modalName, $.operate.chengjiUrl(id));
			},
			// 评分信息 全屏
			chengjiFull: function(id) {
				table.set();
				var url = "/404.html";
				if ($.common.isNotEmpty(id)) {
					url = table.options.chengjiUrl.replace("{id}", id);
				} else {
					var row = $.common.isEmpty(table.options.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns(table.options.uniqueId);
					url = table.options.chengjiUrl.replace("{id}", row);
				}
				$.modal.openFull("评分" + table.options.modalName, url);
			},
			// 评分地址
			chengjiUrl: function(id) {
				var url = "/404.html";
				if ($.common.isNotEmpty(id)) {
					url = table.options.chengjiUrl.replace("{id}", id);
				} else {
			...
			zt: function(id) {
				...
				if($.common.isEmpty(id) && table.options.type == table_type.bootstrapTreeTable) {
					...
					var url = table.options.ztUrl.replace("{id}", row[table.options.uniqueId]);
					$.modal.open("统计" + table.options.modalName, url);
				} else {
					$.modal.open("统计" + table.options.modalName, $.operate.ztUrl(id));
				}
			},
			// 统计信息，以tab页展现
			ztTab: function(id) {
				table.set();
				$.modal.openTab("统计" + table.options.modalName, $.operate.ztUrl(id));
			},
			// 统计信息 全屏
			ztFull: function(id) {
				table.set();
				var url = "/404.html";
				if ($.common.isNotEmpty(id)) {
					url = table.options.ztUrl.replace("{id}", id);
				} else {
					var row = $.common.isEmpty(table.options.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns(table.options.uniqueId);
					url = table.options.ztUrl.replace("{id}", row);
				}
				$.modal.openFull("统计" + table.options.modalName, url);
			},
			// 统计地址
			ztUrl: function(id) {
				var url = "/404.html";
